...
# uses what the second left. Run one alone with -f and it fails.
#
...
# The handlers the CRI reports it can serve, one per line. The kubelet and the
# scheduler work from this list, so it is where a stale handler does harm.
...
CONTAINERD_CONFIG_DIRS="$(containerd_config_roots) /host/opt/kata/containerd"
...
@test "Debug variant is registered while debug is on" {
	...
	# Also the baseline for the next test: no hit here would make its absence
	# there prove nothing.
...
@test "Turning debug off withdraws the variant handler" {
	...
	# Its configuration is gone, so the handler has no business being
	# registered anywhere.
	run containerd_configs_naming "${DEBUG_HANDLER}"
	echo "# containerd configs naming ${DEBUG_HANDLER}: ${output}" >&3
	[[ "${output}" == "NONE" ]]

	# A prune that took the base shim too would be just as broken.
