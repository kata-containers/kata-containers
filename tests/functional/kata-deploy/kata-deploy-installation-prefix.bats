...
#
# A prefix other than /opt/kata makes kata-deploy rewrite every shim's artifact
# paths, and a shim it gets wrong is handed artifacts it never asked for.
# qemu-coco-dev is the confidential config that needs no TEE hardware to test
# that with. (Regression test for #13697)
#
...
EOF
	...
	# Reading the host costs a pod per file, so collect once and reuse.
	mkdir -p "${DUMP_DIR}"
...
# line: one per shim, plus one per custom runtime handler.
runtime_config_dirs() {
...
# A runtime directory's base configuration and its prefix drop-in, separated by
# ${DUMP_SEPARATOR}. Chained, so a missing file fails here rather than handing
# the tests an empty configuration to find nothing wrong with.
runtime_configs() {
	local dir="${1}"
...
@test "The prefix drop-in moves the artifacts each configuration asks for" {
	...
	for name in ${names}; do
		...
		# directory, so failing this means we never read the base config.
		...
		for key in "${ARTIFACT_KEYS[@]}"; do
			...
			if [[ "${base_value}" == "${DEFAULT_INSTALL_DIR}/"* ]]; then
				...
				# Nothing of ours to move, so naming the key would hand the
				# shim an initrd or a firmware path it never asked for.
				[[ -z "${drop_in_value}" ]]
			fi
		done
	done

	# A deploy that left the confidential shim out would pass everything above.
	echo "# ${CONFIDENTIAL_SHIM} configured: ${saw_confidential}" >&3
	[[ "${saw_confidential}" == "yes" ]]
}

@test "Every path the prefix drop-in sets is on the host" {
	# The runtime resolves each of them, so a missing path is a boot failure.
	local checks=""
	local name
	for name in $(ls "${DUMP_DIR}"); do
		local configs base drop_in path unresolved
		configs=$(cat "${DUMP_DIR}/${name}")
		base="${configs%%"${DUMP_SEPARATOR}"*}"
		drop_in="${configs##*"${DUMP_SEPARATOR}"}"

		# A shim sharing nothing never execs virtiofsd, so the payload is free
		# not to ship it.
		unresolved=""
		if [[ "$(toml_value "${base}" "shared_fs")" == "none" ]]; then
			unresolved=$(toml_value "${drop_in}" "virtio_fs_daemon")
		fi

		for path in $(toml_paths "${drop_in}"); do
			if [[ -n "${unresolved}" && "${path}" == "${unresolved}" ]]; then
				echo "# ${name}: ${path} (shared_fs = none, not resolved)" >&3
				continue
			fi
