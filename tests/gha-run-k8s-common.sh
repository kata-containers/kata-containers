...
# Full k3s/RKE2 V3 containerd template (from k3s pkg/agent/templates/templates.go
# ContainerdConfigTemplateV3). Placeholder __CONTAINERD_IMPORTS_PATH__ is replaced
# with the actual drop-in path for k3s or RKE2.
readonly CONTAINERD_V3_TEMPLATE='{{- /* */ -}}
...
# the full V3 template with the given import path and restart the service.
...
function _setup_containerd_v3_template_if_needed() {
	...
	echo "${CONTAINERD_V3_TEMPLATE}" | sed "s|__CONTAINERD_IMPORTS_PATH__|${imports_path}|g" | sudo tee "${containerd_dir}/config-v3.toml.tmpl" > /dev/null
