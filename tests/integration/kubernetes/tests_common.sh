...
# adapt common policy settings for mariner
adapt_common_policy_settings_for_mariner() {
	local settings_dir=$1

	info "Adapting common policy settings for Mariner"
...
adapt_common_policy_settings() {
	...
	case "${KATA_HOST_OS}" in
  		"cbl-mariner")
			adapt_common_policy_settings_for_mariner "${settings_dir}"
			;;
	esac
}
