...
setup() {
    # TODO - do TEEs want to run these tests?
...
function setup_kbs_credentials() {
    ...
    if [[ -z ${user} || -z ${password} ]]; then
        if [[ -n ${GITHUB_ACTION} ]]; then
            ...
            skip "running local and missing user/password"
        fi
    fi
...
# TODO - remove runc test once debug is over
@test "Debug: Test that creating a runc container from an authenticated image, with correct credentials works" {

    kata_pod_with_private_image="$(new_pod_config "${AUTHENTICATED_IMAGE}" "runc")"
    set_node "${kata_pod_with_private_image}" "${node}"
    set_container_command "${kata_pod_with_private_image}" "0" "sleep" "30"
    yq -i ".spec.imagePullSecrets[0].name = \"cococred\"" "${kata_pod_with_private_image}"
...
@test "Test that creating a container from an authenticated image, with incorrect credentials fails" {
    ...
    assert_logs_contain "${node}" kata "${node_start_time}" "failed to pull manifest Not authorize"
}
...
@test "Test that creating a container from an authenticated image, with no credentials fails" {
    ...
    setup_image_pull_secret_for_nydus_snapshotter_metadata_pull "${kata_pod_with_private_image}"
...
teardown() {
    ...
    # TODO - where is pod_name supposed to be set?
