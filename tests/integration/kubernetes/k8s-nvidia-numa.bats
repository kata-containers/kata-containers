...
# numa_unconfigured_reason returns a non-empty reason on stdout when the
# hypervisor under test does not enable NUMA by default, and nothing when
# it does.  Tests that hold on any host NUMA topology use this one.
# Callers must invoke `skip` themselves — bats `skip` inside command
# substitution does not propagate.
numa_unconfigured_reason() {
...
# numa_skip_reason extends numa_unconfigured_reason with the >= 2 host NUMA
# nodes a test needs to tell node-local placement from node-remote.
numa_skip_reason() {
    local reason
    reason=$(numa_unconfigured_reason)
    if [[ -n "${reason}" ]]; then
        echo "${reason}"
        return 0
    fi
...
@test "NUMA: two sandboxes do not claim the same host CPUs" {
    ...
    local skip_reason
    skip_reason=$(numa_unconfigured_reason)
    [[ -z "${skip_reason}" ]] || skip "${skip_reason}"
