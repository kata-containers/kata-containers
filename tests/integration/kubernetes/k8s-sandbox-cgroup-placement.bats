...
host_cgroup_v2_path() {
	local pid="${1}"
	exec_host "${node}" "cat /proc/${pid}/cgroup" | awk -F'::' '/^0::/ {print $2}'
}
...
@test "runtime, virtiofsd and VMM share the pod sandbox cgroup" {
	...
	[[ -n "${pod_uid}" ]] || die "could not read UID for pod ${pod_name}"

	local shim_pid vmm_pid vfsd_pid
	shim_pid="$(shim_pid_for_pod "${node}" "${pod_uid}")"
	[[ -n "${shim_pid}" ]] || die "could not find the kata shim for pod ${pod_name} (uid ${pod_uid}) on ${node}"

	vmm_pid="$(exec_host "${node}" "pgrep -P ${shim_pid} -f 'cloud-hypervisor|qemu-system|firecracker|stratovirt' | head -1")"
	[[ -n "${vmm_pid}" ]] || die "could not find the VMM (child of shim ${shim_pid}) on ${node}"

	vfsd_pid="$(exec_host "${node}" "pgrep -P ${shim_pid} -f virtiofsd | head -1")"
	...
	[[ -n "${shim_cgroup}" ]] || die "could not read cgroup v2 path for shim pid ${shim_pid}"
	...
	if [[ -n "${vfsd_pid}" ]]; then
		...
		[[ -n "${vfsd_cgroup}" ]] || die "could not read cgroup v2 path for virtiofsd pid ${vfsd_pid}"
