...
# "kill 0" on SIGTERM (the same pattern as the original wordpress reproducer).
# After deletion it asserts the pod disappears within the termination grace
# period, proving the CRI engine handled the dead-shim case cleanly.
#
# References:
#   https://github.com/kata-containers/kata-containers/issues/XXXX
#   kata commit 956f43c6c ("runtime: skip MoveTo for systemd cgroups")
...
# stuck in Terminating.  Fails if kubectl delete does not complete within the
# pod's terminationGracePeriodSeconds (30s) plus a small buffer.
@test "Multi-process pod terminates cleanly (no stuck-in-Terminating regression)" {
	...
	# Delete and wait for the pod object to disappear.
	# --timeout must be > terminationGracePeriodSeconds (30s) so kubelet has
	# time to run the full graceful-stop sequence before we declare failure.
