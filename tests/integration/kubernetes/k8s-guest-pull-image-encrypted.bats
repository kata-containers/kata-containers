...
@test "Test that creating a container from an encrypted image, with no decryption key fails" {
    ...
    # TODO - ideally in future we'd have a delete kbs resource to ensure there is no key here
    ...
    #TODO - what are the logs?
    assert_logs_contain "${node}" kata "${node_start_time}" "Can't decrypt"
}
