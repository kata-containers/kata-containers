...
# Normalized for completeness but intentionally NOT propagated to the Trustee
# pods: the runner's NO_PROXY carries ".intel.com", which would also exclude the
# public Intel DCAP collateral endpoint from the proxy and break TDX quote
# verification. See the proxy handling in deploy_kbs() for details.
...
#
# Admin requests are sent unauthenticated; the Helm chart configures KBS with
# admin authorization_mode = "InsecureAllowAll", so no admin token is required.
#
# Parameters:
...
function kbs_k8s_deploy() {
	...
	#
	# NOTE: we deliberately do NOT inherit the runner's ${NO_PROXY} here. On the
	# Intel TDX CI that variable contains ".intel.com,intel.com", which also
	# matches the public Intel DCAP collateral endpoint used by the Attestation
	# Service (api.trustedservices.intel.com, configured in the chart's
	# sgx_default_qcnl.conf). Inheriting it makes the AS try to reach the PCS
	# *directly* from the pod network (bypassing proxy-dmz), which fails with
	# SGX_QL_NETWORK_ERROR (0xe019) -> TDX quote verification fails -> RCAR
	# attest 401 -> guests cannot unwrap image-decryption keys. We therefore
	# scope NO_PROXY to in-cluster destinations only (covers KBS<->AS<->RVPS via
	# .svc / the 10.0.0.0/8 service+pod CIDRs) and let everything external,
	# including the Intel PCS, go through the proxy (matching the previous
	# kustomize deployment, which set https_proxy without any no_proxy).
	local kbs_env_idx=0
	if [[ -n "${HTTPS_PROXY}" || -n "${HTTP_PROXY}" ]]; then
		local merged_no_proxy="${_trustee_default_no_proxy}"
