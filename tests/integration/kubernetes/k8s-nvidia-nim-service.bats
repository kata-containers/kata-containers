...
NGC_API_KEY_SEALED_SECRET_LLAMA_3_2_1B_PAYLOAD=$(
    echo -n '{"version":"0.1.0","type":"vault","name":"kbs:///default/ngc-api-key/llama-3-2-1b","provider":"kbs","provider_settings":{},"annotations":{}}' |
        base64 -w0 | tr '+/' '-_' | tr -d '='
)
NGC_API_KEY_SEALED_SECRET_LLAMA_3_2_1B="sealed.fakejwsheader.${NGC_API_KEY_SEALED_SECRET_LLAMA_3_2_1B_PAYLOAD}.fakesignature"
...
install_nim_operator() {
	...
	if [[ -n "${deploy_name}" ]]; then
		kubectl wait --for=condition=available --timeout=300s "deployment/${deploy_name}" -n "${NIM_OPERATOR_NAMESPACE}" || true
	fi
...
setup_kbs_credentials() {
    ...
    kbs_set_resource "default" "ngc-api-key" "llama-3-2-1b" "${NGC_API_KEY}"
}
