...
if [[ "${ovmf_build}" == "sev" ]]; then
	...
	# The CCA firmware is installed under distinct names so it can ship
	# side by side with the stock arm64 AAVMF firmware (share/aavmf) without
	# clobbering it; the qemu-cca config points at these explicitly.
	install "${build_root}"/"${ovmf_dir}"/"${build_path_fv}"/QEMU_EFI.fd "${install_dir}/AAVMF_CODE.cca.fd"
	install "${build_root}"/"${ovmf_dir}"/"${build_path_fv}"/QEMU_VARS.fd "${install_dir}/AAVMF_VARS.cca.fd"
	# QEMU expects 64MiB CODE and VARS files on ARM/AARCH64 architectures
	# Truncate the firmware files to the expected size
	truncate -s 64M "${install_dir}/AAVMF_CODE.cca.fd"
	truncate -s 64M "${install_dir}/AAVMF_VARS.cca.fd"
