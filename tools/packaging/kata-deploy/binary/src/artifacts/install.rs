...
const QEMU_MICROVM_ARTIFACT: &str = "qemu-microvm";
...
fn write_common_drop_ins(
    ...
    // 1b. Machine type, when this shim is served by the microvm qemu build
    if get_qemu_artifact_name(config, shim).as_deref() == Some(QEMU_MICROVM_ARTIFACT) {
        info!("  - Hypervisor: microvm");
        let microvm_content = generate_qemu_microvm_drop_in(config, shim)?;
        write_drop_in_file(config_d_dir, "15-qemu-microvm.toml", &microvm_content)?;
    } else {
        reconcile_stale_drop_in(config_d_dir, "15-qemu-microvm.toml")?;
    }
...
/// Point a shim at the microvm qemu build.
///
/// Written after 10-installation-prefix.toml so it also wins over the path that
/// one sets for a non-default install prefix.
fn generate_qemu_microvm_drop_in(config: &Config, shim: &str) -> Result<String> {
    ...
    Ok(format!(
        "# Generated by kata-deploy\n\n\
         ...
         valid_hypervisor_paths = [\"{hypervisor_path}\"]\n\
...
fn get_qemu_artifact_name(config: &Config, shim: &str) -> Option<String> {
    if !is_qemu_shim(shim) {
        return None;
    }

    if shim == "qemu-nvidia-cpu-runtime-rs" && !config.qemu_nvidia_cpu_vfio {
        return Some(QEMU_MICROVM_ARTIFACT.to_string());
    }
...
fn create_qemu_wrapper_script(config: &Config, shim: &str) -> Result<Option<String>> {
    let qemu_artifact = match get_qemu_artifact_name(config, shim) {
...
mod tests {
    ...
    fn test_get_qemu_artifact_name(#[case] shim: &str, #[case] expected: &str) {
        ...
        assert_eq!(get_qemu_artifact_name(&config, shim).unwrap(), expected);
    }
    ...
    fn test_get_qemu_artifact_name_non_qemu(#[case] shim: &str) {
        let config = kernel_params_config();
        assert!(get_qemu_artifact_name(&config, shim).is_none());
    }

    #[rstest]
    #[case("qemu-nvidia-cpu-runtime-rs", "qemu-microvm")]
    #[case("qemu-nvidia-gpu-runtime-rs", "qemu-no-shared-fs")]
    fn test_get_qemu_artifact_name_without_vfio(#[case] shim: &str, #[case] expected: &str) {
        let mut config = kernel_params_config();
        config.qemu_nvidia_cpu_vfio = false;
        assert_eq!(get_qemu_artifact_name(&config, shim).unwrap(), expected);
    }
