...
fn write_common_drop_ins(
    ...
    if config.debug {
        ...
        let debug_content = generate_debug_drop_in(config, shim)?;
...
/// Read dm-verity params for a guest extension from the installed root-hash file.
/// The file is a single line: `root_hash=...,salt=...,data_blocks=...,...`.
fn read_extension_verity_params(config: &Config, variant: &str) -> Option<String> {
    let path = format!(
        "{}/share/kata-containers/root_hash_{}.txt",
        config.host_install_dir, variant
    );
...
/// Guest mount name and build variant for the NVIDIA devkit (unchiseled distro tooling) extension.
const DEVKIT_EXTENSION_NAME: &str = "devkit";
const DEVKIT_DEBUG_CONSOLE_SHELL: &str = "/run/kata-extensions/devkit/bin/sh";
const NVIDIA_DEVKIT_EXTENSION_VARIANT: &str = "nvidia-devkit-extension";
const NVIDIA_DEVKIT_IMAGE_NAME: &str = "kata-containers-nvidia-devkit-extension.img";

/// Returns true for NVIDIA runtime-rs shims that use the composable nvidia base
/// (shell-less busybox) and can mount the optional devkit extension.
fn is_nvidia_runtime_rs_shim(shim: &str) -> bool {
    matches!(
        shim,
        "qemu-nvidia-cpu-runtime-rs"
            | "qemu-nvidia-gpu-runtime-rs"
...
/// For NVIDIA runtime-rs shims, also cold-plugs the devkit extension image.
/// Note: kernel_params for debug are handled separately in generate_kernel_params_drop_in
fn generate_debug_drop_in(config: &Config, shim: &str) -> Result<String> {
    let hypervisor_name = get_hypervisor_name(shim)?;
...
"#
    ...
    if is_nvidia_runtime_rs_shim(shim) {
        content.push_str(&format!(
            "debug_console_shell = \"{DEVKIT_DEBUG_CONSOLE_SHELL}\"\n"
        ));

        let devkit_image = format!(
            "{}/share/kata-containers/{}",
            config.dest_dir, NVIDIA_DEVKIT_IMAGE_NAME
        );
        match read_extension_verity_params(config, NVIDIA_DEVKIT_EXTENSION_VARIANT) {
            Some(verity_params) => {
                info!(
                    "  - NVIDIA devkit extension: using verity params from root_hash_{}.txt",
                    NVIDIA_DEVKIT_EXTENSION_VARIANT
                );
                content.push_str(&format!(
                    r#"
# Unchiseled distro tooling for kata-runtime exec (development only)
[[hypervisor.{hypervisor_name}.guest_extension_images]]
name = "{DEVKIT_EXTENSION_NAME}"
path = "{devkit_image}"
...
"#
            ...
            None => {
                info!(
                    "  - NVIDIA devkit extension: root_hash_{}.txt not found under {}/share/kata-containers/; \
                     skipping cold-plug (build rootfs-image-nvidia-devkit-extension-tarball and redeploy)",
                    NVIDIA_DEVKIT_EXTENSION_VARIANT, config.dest_dir
                );
            }
        }
    }
...
mod tests {
    ...
    #[rstest]
    #[case("qemu-nvidia-gpu-runtime-rs", true)]
    #[case("qemu-nvidia-gpu-snp-runtime-rs", true)]
    #[case("qemu-nvidia-gpu-tdx-runtime-rs", true)]
    #[case("qemu-nvidia-cpu-runtime-rs", true)]
    #[case("qemu-nvidia-gpu", false)]
    #[case("qemu-runtime-rs", false)]
    fn test_is_nvidia_runtime_rs_shim(#[case] shim: &str, #[case] expected: bool) {
        assert_eq!(is_nvidia_runtime_rs_shim(shim), expected);
    }

    fn test_config(dest_dir: &str) -> Config {
        Config {
            node_name: "test-node".into(),
            debug: true,
            shims_for_arch: vec!["qemu-nvidia-gpu-runtime-rs".into()],
            default_shim_for_arch: "qemu-nvidia-gpu-runtime-rs".into(),
            ...
            host_install_dir: format!("/host{dest_dir}"),
            ...
            daemonset_name: "kata-deploy".into(),
    ...
    #[test]
    fn test_generate_debug_drop_in_nvidia_runtime_rs_includes_extension() {
        let tmp = tempfile::TempDir::new().unwrap();
        let host_install = tmp.path().join("host/opt/kata");
        let share = host_install.join("share/kata-containers");
        fs::create_dir_all(&share).unwrap();
        fs::write(
            share.join("root_hash_nvidia-devkit-extension.txt"),
            "root_hash=abc123,salt=def456,data_blocks=96512,data_block_size=4096,hash_block_size=4096",
        )
        .unwrap();

        let mut config = test_config("/opt/kata");
        config.host_install_dir = host_install.to_string_lossy().into_owned();

        let content = generate_debug_drop_in(&config, "qemu-nvidia-gpu-runtime-rs").unwrap();
        assert!(content.contains("debug_console_enabled = true"));
        assert!(content.contains("debug_console_shell = \"/run/kata-extensions/devkit/bin/sh\""));
        assert!(content.contains("name = \"devkit\""));
        assert!(content.contains(
            "path = \"/opt/kata/share/kata-containers/kata-containers-nvidia-devkit-extension.img\""
        ));
        assert!(content.contains(
            "verity_params = \"root_hash=abc123,salt=def456,data_blocks=96512,data_block_size=4096,hash_block_size=4096\""
        ));
    }

    #[test]
    fn test_generate_debug_drop_in_non_nvidia_omits_extension() {
        let config = test_config("/opt/kata");
        let content = generate_debug_drop_in(&config, "qemu-runtime-rs").unwrap();
        assert!(content.contains("debug_console_enabled = true"));
        assert!(!content.contains("debug_console_shell"));
        assert!(!content.contains("guest_extension_images"));
    }

    #[test]
    fn test_generate_debug_drop_in_nvidia_runtime_rs_skips_extension_without_root_hash() {
        let tmp = tempfile::TempDir::new().unwrap();
        let host_install = tmp.path().join("host/opt/kata/share/kata-containers");
        fs::create_dir_all(&host_install).unwrap();

        let mut config = test_config("/opt/kata");
        config.host_install_dir = tmp
            .path()
            .join("host/opt/kata")
            .to_string_lossy()
            .into_owned();
