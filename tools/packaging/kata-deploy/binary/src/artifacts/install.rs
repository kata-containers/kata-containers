...
/// `native` needs an image opened with O_DIRECT, which is up to
/// `block_device_cache_direct`.
const BLOCK_DEVICE_AIO_FALLBACK: &str = "threads";

const BLOCK_DEVICE_AIO_DROP_IN: &str = "15-block-device-aio.toml";
...
fn write_common_drop_ins(
    ...
    // 1b. Block device AIO engine, when the host has no io_uring
    if is_qemu_shim(shim) {
        match host_io_uring_unavailable_reason() {
            Some(reason) => {
                warn!(
                    "  - Block device AIO: using \"{}\" instead of \"io_uring\": {}",
                    BLOCK_DEVICE_AIO_FALLBACK, reason
                );
                let aio_content = generate_block_device_aio_drop_in(shim, &reason)?;
                write_drop_in_file(config_d_dir, BLOCK_DEVICE_AIO_DROP_IN, &aio_content)?;
            }
            None => reconcile_stale_drop_in(config_d_dir, BLOCK_DEVICE_AIO_DROP_IN)?,
        }
    }
...
/// RHEL 10 and its rebuilds ship `kernel.io_uring_disabled = 2`.
fn host_io_uring_unavailable_reason() -> Option<String> {
    io_uring_unavailable_reason(Path::new(IO_URING_DISABLED_SYSCTL))
}

fn io_uring_unavailable_reason(sysctl: &Path) -> Option<String> {
    let sysctl_path = sysctl.display();

    match read_io_uring_disabled_sysctl(sysctl) {
        Some(0) => return None,
        // 1 allows CAP_SYS_ADMIN, and the VMM is started by a root shim.
        Some(1) => {
            info!(
                "{sysctl_path} is 1: io_uring is restricted to privileged processes, which \
                 the VMM is, so io_uring stays in use"
            );
            return None;
        }
        Some(2) => {
            return Some(format!(
                "{sysctl_path} is 2, so io_uring_setup() is refused for every process on \
                 this host"
            ))
        }
        Some(unknown) => warn!(
            "unrecognised value {unknown} in {sysctl_path}; asking the kernel for a ring \
             instead"
        ),
        // No sysctl (kernel < 6.6) says nothing either way.
        None => {}
    }

    match probe_io_uring_setup() {
        Ok(()) => None,
        Err(err) => Some(format!("io_uring_setup() failed on this host: {err}")),
    }
}

fn read_io_uring_disabled_sysctl(sysctl: &Path) -> Option<u8> {
    fs::read_to_string(sysctl).ok()?.trim().parse().ok()
}

fn probe_io_uring_setup() -> std::io::Result<()> {
    // sizeof(struct io_uring_params): the kernel copies all 120 bytes in and out.
    let mut params = [0u64; 15];

    let ring = unsafe { libc::syscall(libc::SYS_io_uring_setup, 1, params.as_mut_ptr()) };
    if ring < 0 {
...
fn generate_block_device_aio_drop_in(shim: &str, reason: &str) -> Result<String> {
    let hypervisor_name = get_hypervisor_name(shim)?;

    Ok(format!(
        r#"# Block device AIO engine
# Generated by kata-deploy
#
# This host cannot serve block_device_aio = "io_uring":
#   {reason}
# Enable io_uring on the host (see {IO_URING_DISABLED_SYSCTL}) and redeploy to
# get it back.

[hypervisor.{hypervisor_name}]
block_device_aio = "{BLOCK_DEVICE_AIO_FALLBACK}"
"#
    ))
}
...
mod tests {
    ...
    #[rstest]
    #[case::missing(None)]
    #[case::garbage(Some("elephant"))]
    fn test_io_uring_falls_back_to_probing_the_kernel(#[case] sysctl: Option<&str>) {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("io_uring_disabled");
        if let Some(content) = sysctl {
            fs::write(&path, content).unwrap();
        }

        // Whatever this host allows is the right answer, either way.
        assert_eq!(
            io_uring_unavailable_reason(&path).is_some(),
            probe_io_uring_setup().is_err()
        );
    }
    ...
    #[case("2", true)]
    #[case("2\n", true)]
    fn test_io_uring_availability_from_sysctl(#[case] content: &str, #[case] unavailable: bool) {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("io_uring_disabled");
        fs::write(&path, content).unwrap();

        assert_eq!(io_uring_unavailable_reason(&path).is_some(), unavailable);
    }

    #[test]
    fn test_generate_block_device_aio_drop_in() {
        let content = generate_block_device_aio_drop_in("qemu-tdx", "the sysctl says no").unwrap();

        let doc = content.parse::<DocumentMut>().unwrap();
        assert_eq!(
            doc["hypervisor"]["qemu"]["block_device_aio"].as_str(),
            Some(BLOCK_DEVICE_AIO_FALLBACK)
        );
        assert!(content.contains("the sysctl says no"));
    }
