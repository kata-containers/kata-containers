...
fn get_hypervisor_name(shim: &str) -> Result<&str> {
    match shim {
        ...
        | "qemu-nvidia-gpu"
        | "qemu-nvidia-gpu-tdx"
        | "qemu-nvidia-gpu-snp" => Ok("qemu"),
        "clh" => Ok("clh"),
        "cloud-hypervisor" => Ok("cloud-hypervisor"),
        "dragonball" => Ok("dragonball"),
        "fc" | "firecracker" => Ok("firecracker"),
        "stratovirt" => Ok("stratovirt"),
        "remote" => Ok("remote"),
        _ => anyhow::bail!(
            "Unknown shim '{}'. Valid shims are: qemu, qemu-tdx, qemu-snp, qemu-se, qemu-coco-dev, qemu-cca, qemu-nvidia-gpu, qemu-nvidia-gpu-tdx, qemu-nvidia-gpu-snp, qemu-runtime-rs, qemu-coco-dev-runtime-rs, qemu-se-runtime-rs, fc, firecracker, clh, cloud-hypervisor, dragonball, stratovirt, remote",
            shim
        ),
    }
}
...
pub async fn install_artifacts(config: &Config) -> Result<()> {
    ...
    if config.create_runtimeclasses {
        runtimeclasses::create_runtimeclasses(config, expand_runtime_classes_for_nfd).await?;
    } else if expand_runtime_classes_for_nfd {
...
pub async fn remove_artifacts(config: &Config) -> Result<()> {
    ...
    if config.create_runtimeclasses {
        runtimeclasses::delete_runtimeclasses(config).await?;
    }
...
fn set_executable_permissions(dir: &str) -> Result<()> {
    ...
    let runtime_rs_bin_dir = Path::new(dir).join("runtime-rs/bin");
    if runtime_rs_bin_dir.exists() {
        for entry in fs::read_dir(&runtime_rs_bin_dir)? {
            let entry = entry?;
            let path = entry.path();
            if path.is_file() {
                let mut perms = fs::metadata(&path)?.permissions();
...
async fn configure_shim_config(config: &Config, shim: &str) -> Result<()> {
    ...
    if !kata_config_file.exists() {
        return Err(anyhow::anyhow!(
            "Configuration file not found: {kata_config_file:?}"
        ));
    }
...
async fn configure_proxy(
    ...
    if !proxy_value.is_empty() {
        ...
        let param_name = if proxy_type == "https_proxy" {
            "agent.https_proxy"
        } else {
            return Ok(());
        };
...
async fn configure_debug(config_file: &Path, shim: &str) -> Result<()> {
    ...
    let current_hypervisor_debug =
        toml_utils::get_toml_value(config_file, &hypervisor_enable_debug_path)
            .unwrap_or_else(|_| "false".to_string());
    if current_hypervisor_debug != "true" {
        log::debug!(
            "Updating {} in {}: old=\"{}\" new=\"true\"",
            hypervisor_enable_debug_path,
            config_file.display(),
            current_hypervisor_debug
        );
        toml_utils::set_toml_value(config_file, &hypervisor_enable_debug_path, "true")?;
    }

    let current_runtime_debug = toml_utils::get_toml_value(config_file, "runtime.enable_debug")
        .unwrap_or_else(|_| "false".to_string());
    if current_runtime_debug != "true" {
        log::debug!(
            "Updating runtime.enable_debug in {}: old=\"{}\" new=\"true\"",
            config_file.display(),
            current_runtime_debug
        );
        toml_utils::set_toml_value(config_file, "runtime.enable_debug", "true")?;
    }

    let current_debug_console =
        toml_utils::get_toml_value(config_file, "agent.kata.debug_console_enabled")
            .unwrap_or_else(|_| "false".to_string());
    if current_debug_console != "true" {
        log::debug!(
            "Updating agent.kata.debug_console_enabled in {}: old=\"{}\" new=\"true\"",
            config_file.display(),
            current_debug_console
        );
        toml_utils::set_toml_value(config_file, "agent.kata.debug_console_enabled", "true")?;
    }

    let current_agent_debug = toml_utils::get_toml_value(config_file, "agent.kata.enable_debug")
        .unwrap_or_else(|_| "false".to_string());
    if current_agent_debug != "true" {
        log::debug!(
            "Updating agent.kata.enable_debug in {}: old=\"{}\" new=\"true\"",
            config_file.display(),
            current_agent_debug
        );
        toml_utils::set_toml_value(config_file, "agent.kata.enable_debug", "true")?;
    }
...
async fn configure_experimental_force_guest_pull(config_file: &Path) -> Result<()> {
    let current_value =
        toml_utils::get_toml_value(config_file, "runtime.experimental_force_guest_pull")
            .unwrap_or_else(|_| "false".to_string());
    if current_value != "true" {
        log::debug!(
            "Updating runtime.experimental_force_guest_pull in {}: old=\"{}\" new=\"true\"",
            config_file.display(),
            current_value
        );
        toml_utils::set_toml_value(config_file, "runtime.experimental_force_guest_pull", "true")?;
    }
    Ok(())
}
...
async fn configure_tdx(config: &Config, _shim: &str, config_file: &Path) -> Result<()> {
    ...
    match id.as_str() {
        "ubuntu" => match version_id.as_str() {
            "24.04" | "25.04" | "25.10" => {
                tdx_supported(config, &id, &version_id, config_file).await?;
            }
            _ => {
                log::warn!(
                        "Distro {id} {version_id} does not support TDX and the TDX related runtime classes will not work in your cluster!"
                    );
            }
        },
        "centos" => {
...
async fn configure_mariner(config: &Config) -> Result<()> {
    ...
    let current_value = toml_utils::get_toml_value(config_file, &static_resource_mgmt_path)
        .unwrap_or_else(|_| "false".to_string());
    if current_value != "true" {
        log::debug!(
            "Updating {} in {}: old=\"{}\" new=\"true\"",
            static_resource_mgmt_path,
            config_file.display(),
            current_value
        );
        toml_utils::set_toml_value(config_file, &static_resource_mgmt_path, "true")?;
    }
...
VERSION_ID="24.04"
"#;
        assert_eq!(extract_os_release_field(content, "ID"), "ubuntu");
        assert_eq!(extract_os_release_field(content, "VERSION_ID"), "24.04");
    }

    #[test]
    fn test_extract_os_release_field_empty() {
        let content = "";
        assert_eq!(extract_os_release_field(content, "ID"), "");
    }

    #[test]
    fn test_extract_os_release_field_missing() {
        let content = "ID=ubuntu\n";
        assert_eq!(extract_os_release_field(content, "VERSION_ID"), "");
    }
    ...
    fn test_get_hypervisor_name_other_hypervisors() {
        ...
        assert_eq!(get_hypervisor_name("stratovirt").unwrap(), "stratovirt");
...
"#,
        ...
        let config = crate::config::Config {
            node_name: "test-node".to_string(),
            debug: false,
            shims_for_arch: vec!["qemu-cca".to_string()],
            default_shim_for_arch: "qemu-cca".to_string(),
            create_runtimeclasses: false,
            create_default_runtimeclass: false,
            allowed_hypervisor_annotations_for_arch: vec![],
            snapshotter_handler_mapping_for_arch: None,
...
"#,
        ...
        let config = crate::config::Config {
            node_name: "test-node".to_string(),
            debug: false,
            shims_for_arch: vec!["qemu-nvidia-gpu-snp".to_string()],
            default_shim_for_arch: "qemu-nvidia-gpu-snp".to_string(),
            create_runtimeclasses: false,
            create_default_runtimeclass: false,
            allowed_hypervisor_annotations_for_arch: vec![],
            snapshotter_handler_mapping_for_arch: None,
...
"#,
        ...
        let config = crate::config::Config {
            node_name: "test-node".to_string(),
            debug: false,
            shims_for_arch: vec!["qemu-nvidia-gpu-tdx".to_string()],
            default_shim_for_arch: "qemu-nvidia-gpu-tdx".to_string(),
            create_runtimeclasses: false,
            create_default_runtimeclass: false,
            allowed_hypervisor_annotations_for_arch: vec![],
            snapshotter_handler_mapping_for_arch: None,
...
"#;
        ...
        let config = crate::config::Config {
            node_name: "test-node".to_string(),
            debug: false,
            shims_for_arch: vec!["fc".to_string()],
            default_shim_for_arch: "fc".to_string(),
            create_runtimeclasses: false,
            create_default_runtimeclass: false,
            allowed_hypervisor_annotations_for_arch: vec![],
            snapshotter_handler_mapping_for_arch: None,
