...
use crate::config::{Config, KernelModulesImageEntry, DEFAULT_KATA_INSTALL_DIR};
use crate::k8s::nfd;
...
fn write_common_drop_ins(
    ...
    // 4. Kernel modules images (per-shim)
    if let Some(images) = config.kernel_modules_images.get(shim) {
        if !images.is_empty() {
            info!(
                "  - Kernel modules images: {} configured for shim '{}'",
                images.len(),
                shim
            );
            let modules_content = generate_kernel_modules_drop_in(images, shim, &config.dest_dir)?;
            write_drop_in_file(config_d_dir, "40-kernel-modules.toml", &modules_content)?;
        }
    }
...
/// Generate drop-in content for kernel modules images.
fn generate_kernel_modules_drop_in(
    images: &[KernelModulesImageEntry],
    shim: &str,
    dest_dir: &str,
) -> Result<String> {
    let hypervisor_name = get_hypervisor_name(shim)?;
    ...
    let mut content = String::new();
    content.push_str("# Kernel modules images configuration\n");
    content.push_str("# Generated by kata-deploy\n\n");

    let mut all_modules: Vec<String> = Vec::new();

    for img in images {
        let path = if dest_dir != DEFAULT_KATA_INSTALL_DIR
            && img.path.starts_with(DEFAULT_KATA_INSTALL_DIR)
        {
            img.path.replace(DEFAULT_KATA_INSTALL_DIR, dest_dir)
        } else {
            img.path.clone()
        };

        content.push_str(&format!(
            "[[hypervisor.{}.kernel_modules_images]]\n",
            hypervisor_name
        ));
        content.push_str(&format!("path = \"{}\"\n", path));
        content.push_str(&format!("verity_params = \"{}\"\n\n", img.verity_params));
    ...
    if !all_modules.is_empty() {
        let modules_toml: Vec<String> = all_modules.iter().map(|m| format!("\"{}\"", m)).collect();
        content.push_str(&format!(
            "[agent.kata]\nkernel_modules = [{}]\n",
            modules_toml.join(", ")
        ));
