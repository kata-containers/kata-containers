...
/// Path to the rendered containerd config (template path with ".tmpl" stripped).
/// This is the file containerd actually reads; we check it for the drop-in import.
pub fn k3s_rke2_rendered_config_path(template_path: &str) -> String {
    template_path
        .strip_suffix(".tmpl")
    ...
    pub async fn get_containerd_paths(&self, runtime: &str) -> Result<ContainerdPaths> {
        ...
        let paths = match runtime {
            ...
            "k3s" | "k3s-agent" | "rke2-agent" | "rke2-server" => {
                ...
                let rendered_path = k3s_rke2_rendered_config_path(&config_file);
...
mod tests {
    ...
    #[rstest]
    #[case("/etc/containerd/config.toml.tmpl", "/etc/containerd/config.toml")]
    #[case("/etc/containerd/config-v3.toml.tmpl", "/etc/containerd/config-v3.toml")]
    #[case("/no/suffix", "/no/suffix")]
    fn test_k3s_rke2_rendered_config_path(#[case] template_path: &str, #[case] expected: &str) {
        assert_eq!(k3s_rke2_rendered_config_path(template_path), expected);
    }
