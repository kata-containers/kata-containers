...
/// Returns None only if neither exists or if the value is empty
...
fn get_arch_var_or_base(base_name: &str, arch: &str) -> Option<String> {
    ...
    // Fall back to base variable, filtering out empty strings
    env::var(base_name).ok().filter(|s| !s.is_empty())
}
