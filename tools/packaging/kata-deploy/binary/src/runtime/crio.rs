...
pub async fn configure_crio_runtime(config: &Config, shim: &str) -> Result<()> {
    ...
    writeln!(file, "\truntime_path = \"{kata_path}\"")?;
    writeln!(file, "\truntime_type = \"vm\"")?;
    writeln!(file, "\truntime_root = \"/run/vc\"")?;
    writeln!(file, "\truntime_config_path = \"{kata_config_path}\"")?;
    writeln!(file, "\tprivileged_without_host_devices = true")?;
    ...
    match config.pull_type_mapping_for_arch.as_ref() {
        Some(mapping) if !mapping.is_empty() => {
            ...
            for m in pull_types {
                ...
                match value {
                    "guest-pull" => writeln!(file, "\truntime_pull_image = true")?,
                    _ => {
...
pub async fn configure_crio(config: &Config) -> Result<()> {
    ...
    writeln!(file, "[crio]")?;
    writeln!(file, "  storage_option = [")?;
    writeln!(file, "\t\"overlay.skip_mount_home=true\",")?;
    writeln!(file, "  ]")?;
    ...
    if config.debug {
        ...
        writeln!(debug_file, "[crio.runtime]")?;
        writeln!(debug_file, "log_level = \"debug\"")?;
    }
