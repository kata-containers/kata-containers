...
async fn install(config: &config::Config, runtime: &str) -> Result<()> {
    ...
    if !matches!(
        ...
        info!("Runtime {runtime} not supported, skipping installation");
        return Ok(());
    }
