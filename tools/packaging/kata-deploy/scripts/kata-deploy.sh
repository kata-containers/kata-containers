...
function configure_containerd() {
	...
	if [ $use_containerd_drop_in_conf_file = "true" ]; then
		if ! grep -q "${containerd_drop_in_conf_file}" ${containerd_root_conf_file}; then
...
function configure_erofs_snapshotter() {
	...
	tomlq -i -t $(printf 'plugins."io.containerd.cri.v1.images"') ${configuration_file}
	tomlq -i -t $(printf 'plugins."io.containerd.cri.v1.images".discard_unpacked_layers=false') ${configuration_file}

	tomlq -i -t $(printf 'plugins."io.containerd.service.v1.diff-service"') ${configuration_file}
	tomlq -i -t $(printf 'plugins."io.containerd.service.v1.diff-service".default=["erofs", "walking"]') ${configuration_file}

	tomlq -i -t $(printf 'plugins."io.containerd.snapshotter.v1.erofs"') ${configuration_file}
...
function configure_nydus_snapshotter() {
	...
	tomlq -i -t $(printf 'plugins.%s' ${pluginid}) ${configuration_file}
	tomlq -i -t $(printf 'plugins.%s.disable_snapshot_annotations=false' ${pluginid}) ${configuration_file}

	tomlq -i -t $(printf 'proxy_plugins') ${configuration_file}
...
function install_nydus_snapshotter() {
	...
	mkdir -p /host/usr/local/share/nydus-snapshotter
