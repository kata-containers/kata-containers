	...
	if [ -n "${SNAPSHOTTER_HANDLER_MAPPING}" ]; then
		for m in "${snapshotters[@]}"; do
			...
			if [[ "${value}" == "nydus" ]] && [[ -n "${MULTI_INSTALL_SUFFIX}" ]]; then
				value="${MULTI_INSTALL_SUFFIX}-${value}"
			fi
...
function configure_nydus_snapshotter() {
	...
	if [[ -n "${MULTI_INSTALL_SUFFIX}" ]]; then
		nydus="${MULTI_INSTALL_SUFFIX}-${nydus}"
		containerd_nydus="${MULTI_INSTALL_SUFFIX}-${containerd_nydus}"
	fi
...
function configure_snapshotter() {
	...
	case "${snapshotter}" in
		nydus)
			...
			if [[ -n "${MULTI_INSTALL_SUFFIX}" ]]; then
				nydus_snapshotter="${MULTI_INSTALL_SUFFIX}-${nydus_snapshotter}"
			fi
