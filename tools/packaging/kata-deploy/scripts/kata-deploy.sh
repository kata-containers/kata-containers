...
function adjust_qemu_cmdline() {
	...
	[[ "${shim}" =~ ^(qemu-snp|qemu-nvidia-gpu-snp|qemu-nvidia-gpu-tdx)$ ]] && qemu_share=${shim}-experimental
...
function install_artifacts() {
	...
	for shim in "${shims[@]}"; do
		...
		if [ "${dest_dir}" != "${default_dest_dir}" ]; then
			...
			[[ "${shim}" =~ ^(qemu|qemu-snp|qemu-nvidia-gpu|qemu-nvidia-gpu-snp|qemu-nvidia-gpu-tdx|qemu-sev|qemu-se|qemu-coco-dev)$ ]] && \
