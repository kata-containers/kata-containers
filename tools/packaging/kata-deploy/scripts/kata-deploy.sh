...
function install_artifacts() {
	...
	if [ "${HOST_OS:-}" == "cbl-mariner" ]; then
		...
		tomlq -i -t '.hypervisor.'"${hypervisor_name}"'.static_sandbox_resource_mgmt = true' "${config_path}" 2>/dev/null || true
		tomlq -i -t '.hypervisor.'"${hypervisor_name}"'.valid_hypervisor_paths = ["'"${clh_path}"'"]' "${config_path}" 2>/dev/null || true
		tomlq -i -t '.hypervisor.'"${hypervisor_name}"'.path = "'"${clh_path}"'"' "${config_path}" 2>/dev/null || true
	fi
