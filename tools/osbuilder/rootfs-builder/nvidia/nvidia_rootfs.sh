...
setup_nvidia_gpu_rootfs_stage_one() {
	...
	local cuda_repo_url cuda_repo_pkg cuda_repo_osv
	...
	cuda_repo_osv=$(get_package_version_from_kata_yaml "assets.image.architecture.x86_64.nvidia-gpu.version")
	...
	chroot . /bin/bash -c "/nvidia_chroot.sh ${machine_arch} ${NVIDIA_GPU_STACK} \
		 ${cuda_repo_osv} ${cuda_repo_url} ${cuda_repo_pkg} ${tools_repo_url} ${tools_repo_pkg}"
