...
{	
	...
	cat <<-'CHROOT_EOF' > /bin/nvidia-persistenced.sh
	#!/bin/bash -x 
	exec &>> /tmp/nvidia-persistenced.log
	
	FLAG_FILE="/tmp/nvidia-persistenced.flag"

	if [ ! -f "$FLAG_FILE" ]; then
		touch $FLAG_FILE
		/usr/bin/nvidia-persistenced --user nobody --group nogroup
	fi
	CHROOT_EOF
	...
	#SUBSYSTEM=="pci", ATTRS{vendor}=="0x10de", DRIVER=="nvidia", RUN+="/bin/lock-run.sh 5 '/usr/bin/nvidia-ctk system create-device-nodes --control-devices --load-kernel-modules && /usr/bin/nvidia-ctk cdi generate --output=/var/run/cdi/nvidia.json'"
	#SUBSYSTEM=="pci", ATTRS{vendor}=="0x10de", DRIVER=="nvidia", RUN+="/bin/nvidia-persistenced.sh"
...
TODO_install_nvidia_verifier_hook() 
{
...
log_time() 
{
    ...
    local exec_time
    exec_time=$(cat temp_time_log | grep real | awk '{print $2}')
...
log_time install_nvidia_fabricmanager
log_time install_nvidia_ctk 
#log_time TODO_install_nvidia_verifier_hook
