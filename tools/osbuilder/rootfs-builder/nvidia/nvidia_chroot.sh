...
EOF
	...
	# Install kernel source package (not nvidia-open which uses DKMS)
	# This allows build_nvidia_drivers() to build with proper kernel signing
	# driver_type is "-open" for open kernel modules or "" for proprietary
	if [[ "${driver_type}" == "-open" ]]; then
		echo "chroot: Installing NVIDIA open kernel source (no DKMS)"
		eval "${APT_INSTALL}" nvidia-kernel-source-open
	else
		echo "chroot: Installing NVIDIA proprietary kernel source (no DKMS)"
		eval "${APT_INSTALL}" nvidia-kernel-source
	fi

	# Verify installed version
	echo "chroot: Verifying installed driver version..."
	dpkg -l | grep -E "nvidia-kernel-source" || true

	# Set driver_version to the actual installed version (major version number)
	driver_version=$(dpkg -l | grep -E "nvidia-kernel-source" | head -1 | awk '{print $3}' | cut -d'.' -f1)
	echo "chroot: Installed driver version: ${driver_version}"
...
get_supported_gpus_from_distro_drivers() {
	...
	if is_feature_enabled "nvidia-repo"; then
		...
		# Find and download the latest nvidia-driver package available
		local nvidia_driver_pkg
		nvidia_driver_pkg=$(apt-cache search --names-only '^nvidia-driver-[0-9]+$' | sort -t'-' -k3 -n | tail -1 | awk '{print $1}')
		echo "chroot: Downloading ${nvidia_driver_pkg} for supported-gpus.json"

		apt download "${nvidia_driver_pkg}"
		ar -x "${nvidia_driver_pkg}"*.deb
		tar -xvf data.tar.*

		local nvidia_driver_version
		nvidia_driver_version=$(echo "${nvidia_driver_pkg}" | sed 's/nvidia-driver-//')
		supported_gpus_json=./usr/share/doc/nvidia-driver-"${nvidia_driver_version}"/supported-gpus.json
