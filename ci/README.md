...
In our CI we have two catagories of jobs - required and not-required:
...
> required/not-required, but they are subject to change and the Kata
...
responsible for the jobs' stability.
...
Once a job has been removed from the required list, due to instability
it required two consecutive successful nightly test runs before being
required again.
