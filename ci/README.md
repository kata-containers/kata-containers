...
- Run the following tests:
  ...
  - Tests depending on the generated kata-deploy payload
    ...
    - kubernetes (runs in Azure small and medium instances
    – depending on what's required by each test, and on TEE bare-metal machines)
...
Deploying vm debug-nerdctl-pr8070 from image ubuntu2304...
debug-nerdctl-pr8070 created on local
```
...
> [!NOTE]
> Those artifacts are only available when all jobs are finished.
...
remote: Enumerating objects: 93672, done.
remote: Counting objects: 100% (9/9), done.
remote: Compressing objects: 100% (8/8), done.
remote: Total 93672 (delta 3), reused 5 (delta 1), pack-reused 93663
Receiving objects: 100% (93672/93672), 93.72 MiB | 9.52 MiB/s, done.
Resolving deltas: 100% (60179/60179), done.
```

Add the upstream as a remote, set up your git, and rebase your branch atop of the upstream's main one
...
kata-static.tar.xz and then take advantage of [local-build script][local-build]. Be aware
...
[local-build]: https://github.com/kata-containers/kata-containers/blob/main/tools/packaging/kata-deploy/local-build/kata-deploy-build-and-upload-payload.sh
