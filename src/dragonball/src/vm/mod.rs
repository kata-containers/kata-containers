...
impl Vm {
    ...
    pub fn start_microvm(
        ...
        mut seccomp_filters: HashMap<ThreadType, BpfProgram>,
    ) -> std::result::Result<(), StartMicroVmError> {
        info!(self.logger, "VM: received instance start command");

        if let Some(process_seccomp_filter) = seccomp_filters.remove(&ThreadType::All) {
            ...
            if let Err(e) = apply_filter_all_threads(&process_seccomp_filter) {
                ...
                info!(
                    self.logger,
                    "VM: process-wide seccomp filters applied {:?}", process_seccomp_filter
                );
            }
        }
        ...
        self.vcpu_manager()
            .map_err(StartMicroVmError::Vcpu)?
            .start_boot_vcpus(seccomp_filters.remove(&ThreadType::Vmm).unwrap_or_default())
            .map_err(StartMicroVmError::Vcpu)?;
