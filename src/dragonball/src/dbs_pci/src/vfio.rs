...
impl Interrupt {
    ...
    fn add_legacy_irq_resource(&mut self, base: u32) {
        self.has_legacy_irq = true;
        self.legacy_irq = base;
        self.resources.append(Resource::LegacyIrq(base));
    }

    fn get_irq_pin(&self) -> u32 {
        if self.has_legacy_irq {
            (PciInterruptPin::IntA as u32) << 8 | self.legacy_irq
        } else {
            0
        }
    }
        ...
        if let Some(msi) = self.msi.as_mut() {
            if let Some(offset) = msi.with_in_range(offset) {
                match data.len() {
                    ...
                    _ => debug!("invalid msi cap read data length!"),
                };
            }
        }
...
impl Region {
    ...
    fn map(
        ...
        for i in 0..self.mmaps.len() {
            ...
            // This change need upstream vfio-ioctls to change
...
struct VfioPciDeviceState<C: PciSystemContext> {
    ...
    regions: Vec<Region>,
    _constraints: Vec<ResourceConstraint>,
...
impl<C: PciSystemContext> VfioPciDeviceState<C> {
    ...
    fn fixup_msix_region(&mut self) {
        ...
        for region in self.regions.iter_mut() {
            ...
            if region.bar_index == msix.cap.table_bir() {
                ...
                let end = align_to_pagesize(start + msix.table_size + PAGE_SIZE - 1);
    ...
    fn register_regions(&mut self, vm: &Arc<VmFd>) -> Result<()> {
        let ctx = self.context.upgrade().unwrap().get_device_manager_context();
        ...
        for region in self.regions.iter_mut() {
            ...
            if !mappable || region.is_sparse_mmap || region.msix_table.is_some() {
                ...
                if let Err(e) = region.trap(&ctx, &mut tx, device.upgrade().unwrap()) {
                    ctx.cancel_tx(tx);
        ...
        for region in self.regions.iter_mut() {
            if region.reg_index == reg_idx {
                if region.mappable() {
                    ...
                    if let Err(e) = region.retrap(&ctx, &mut tx, device.upgrade().unwrap(), &params)
                    {
                        ctx.cancel_tx(tx);
                        return Err(e);
                    }
...
impl<C: PciSystemContext> VfioPciDevice<C> {
    ...
    pub fn activate(&self, device: Weak<dyn DeviceIo>, resources: DeviceResources) -> Result<()> {
        ...
        while reg_idx < state.regions.len() {
            if state.regions[reg_idx].mappable() {
                state.regions[reg_idx].mmaps[0].slot = all_kvm_slot.pop().unwrap();
                if state.regions[reg_idx].mmaps.len() == 2 {
                    state.regions[reg_idx].mmaps[1].slot = all_kvm_slot.pop().unwrap();
                }
            }
        ...
        let irq_manager = state.context.upgrade().unwrap().get_interrupt_manager();
        ...
        if let Err(e) = state.register_regions(&self.vm_fd) {
            ...
            let _ = state.unregister_regions(&self.vm_fd).map_err(|_e| {
                panic!("failed to rollback changes of VfioPciDevice::register_regions()");
            });
        }
        ...
        debug!(
            ...
            state.vfio_path(),
            state.interrupt.has_legacy_irq
        );
    ...
    pub fn bus_id(&self) -> u8 {
        self.state().bus.upgrade().unwrap().bus_id()
    }
...
impl<C: 'static + PciSystemContext> PciDevice for VfioPciDevice<C> {
    ...
    fn write_config(&self, offset: u32, data: &[u8]) {
        ...
        // TODO: detect change in COMMAND register
            ...
            || reg_idx == PCI_ROM_EXP_BAR_INDEX
        {
            ...
            // TODO: check whether following comment is correct.
