...
pub enum Error {
    ...
    ParseCpuInfo(#[from] crate::cpu::Error),
}
...
pub fn get_node_id(cpu: u32) -> Result<u32> {
    ...
    if let Ok(dirs) = path.read_dir() {
        ...
        return Err(Error::InvalidCpu(cpu));
    }
...
pub fn get_node_map(cpus: &str) -> Result<HashMap<u32, Vec<u32>>> {
    ...
    let cpus_list = split_cpus(cpus)?;

    for c in cpus_list.iter() {
...
pub fn get_numa_nodes() -> Result<HashMap<u32, u32>> {
    ...
    if let Ok(dirs) = numa_node_path.read_dir() {
        for d in dirs.flatten() {
            if let Ok(file_name) = d.file_name().into_string() {
                if file_name.starts_with(NUMA_NODE_PREFIX) {
                    ...
                    match index_string.parse::<u32>() {
                        ...
                        Err(_e) => return Err(Error::InvalidNodeFileName(file_name.to_string())),
                    }
                }
            }
        }
...
fn read_cpu_info_from_node(
    ...
    let split_cpus = split_cpus(cpu_list_string.trim())?;
    info!(
        sl!(),
        "node {} list {:?} from {}", node_index, split_cpus, &cpu_list_string
    );
    for split_cpu_id in split_cpus {
