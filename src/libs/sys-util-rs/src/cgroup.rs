...
pub fn update_disabled_cgroup_list(hierarchies: &[String]) {
    ...
    for hierarchy in hierarchies {
        ...
        match hierarchy.as_str() {
            ...
            _ => info!(sl!(), "unknown cgroup controller {}", hierarchy),
        }
    }
    info!(
        sl!(),
...
impl V1Customized {
    ...
    ///
    /// The `controllers` array configures the enabled subsystems.
    ///
    /// Note :
    /// 1. blkio should before mem
    /// 2. cpu, cpuset, cpuacct should be adjecent to each otherogether
...
mod tests {
    ...
    fn test_get_hierarchy() {
        ...
        // TODO: pub fn get_hierarchy_by_path(path: &str)
    }
