...
impl Runtime {
    ...
    pub fn is_experiment_enabled(&self, feature: &str) -> bool {
        for f in self.experimental.iter() {
            if f == feature {
                return true;
            }
        }
...
"#;
        ...
        assert_eq!(config.runtime.disable_new_netns, true);
        assert_eq!(config.runtime.sandbox_bind_mounts.len(), 0);
        assert_eq!(config.runtime.sandbox_cgroup_only, true);
        assert_eq!(config.runtime.enable_tracing, true);
        assert_eq!(config.runtime.is_experiment_enabled("a"), true);
        assert_eq!(config.runtime.is_experiment_enabled("b"), true);
        assert_eq!(config.runtime.is_experiment_enabled("c"), false);
    }
}
