...
impl FromStr for ContainerType {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            POD_CONTAINER => Ok(ContainerType::PodContainer),
            POD_SANDBOX => Ok(ContainerType::PodSandbox),
...
mod tests {
    ...
    #[test]
    fn test_container_type_try_from() {
        assert_eq!(
    ...
    fn test_valid_transition() {
        ...
        assert!(state.check_transition(new_state).is_err());

        new_state = State::Running;
        assert!(state.check_transition(new_state).is_ok());
