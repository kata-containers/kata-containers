...
/// Block device specific annotations for num_queues
pub const KATA_ANNO_CFG_HYPERVISOR_BLOCK_DEVICE_NUM_QUEUES: &str =
    "io.katacontainers.config.hypervisor.block_device_num_queues";
/// Block device specific annotations for queue_size
pub const KATA_ANNO_CFG_HYPERVISOR_BLOCK_DEVICE_QUEUE_SIZE: &str =
        ...
        for (key, value) in &self.annotations {
            if hv.security_info.is_annotation_enabled(key) {
                match key.as_str() {
                    ...
                    KATA_ANNO_CFG_HYPERVISOR_BLOCK_DEVICE_NUM_QUEUES => {
                    ...
                    KATA_ANNO_CFG_HYPERVISOR_BLOCK_DEVICE_QUEUE_SIZE => {
                        match self.get_value::<u32>(key) {
                            Ok(v) => {
