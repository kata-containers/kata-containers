...
pub fn update_ocispec_annotations(
    ...
    let mut updated = HashMap::with_capacity(
...
mod tests {
    ...
    fn test_update_annotations_not_in_annotations_map() {
        ...
        assert_eq!(updated.get("key1"), Some(&"new_val1".to_string())); // Not overwritten
        assert_eq!(updated.get("key2"), Some(&"val3".to_string())); // keep it
        assert_eq!(updated.get("key3"), Some(&"val3".to_string())); // Added
    }
}
