...
impl InitData {
    ...
    /// read config content and insert it
    pub fn build_initdata<'a>(&mut self, config_name: &'a str, config_path: &'a str) -> Result<()> {
        let config_content = fs::read_to_string(config_path)?;
        self.insert_data(config_name, config_content);
...
/// Validate initdata
fn validate_initdata(initdata_str: &str) -> Result<InitData> {
...
pub fn calculate_initdata_digest(
    ...
    let initdata: InitData =
        validate_initdata(initdata_toml).map_err(|e| anyhow!("validate initdata {:?}", e))?;
    let algorithm: &str = &initdata.algorithm;

    // 2. Calculate Digest
    let digest = calculate_digest(algorithm, &initdata_toml)
        .map_err(|e| anyhow!("calculate digest {:?}", e))?;
...
/// and then calculates its digest using the specified HASH algorithm.
pub fn add_hypervisor_initdata_overrides(initda_annotation: &str) -> Result<String> {
    // Base64 decode the annotation value
    let b64_decoded = base64::decode_config(initda_annotation, base64::STANDARD)
    ...
    gz_decoder
        ...
        .map_err(|e| anyhow!("gz decoder failed {:?}", e))?;

    // Parse the initdata and validate it
    let initdata: InitData =
        validate_initdata(&initdata_str).map_err(|e| anyhow!("parse initdata {:?}", e))?;
...
mod tests {
    ...
    fn test_hypervisor_initdata_processing() {
        ...
        let mut init_data = InitData::new("sha512", "1.0.0");
    ...
    fn test_initdata_validation() {
        ...
        "#;
        assert!(validate_initdata(valid_toml).is_ok());
        ...
        let invalid_toml = r#"
            ...
            key = "value"
        "#;
        assert!(validate_initdata(invalid_toml).is_err());
    }
