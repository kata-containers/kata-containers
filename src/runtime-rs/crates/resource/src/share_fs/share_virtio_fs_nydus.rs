...
use super::share_virtio_fs::{generate_sock_path, prepare_virtiofs, FS_TYPE_VIRTIO_FS, KATA_VIRTIO_FS_DEV_TYPE, MOUNT_GUEST_TAG};
...
impl ShareVirtioFsNydus {
    ...
    fn generate_api_sock_path(&self, root: &str) -> String {
        let socket_path = std::path::Path::new(root).join(NYDUSD_API_SOCK);
    ...
    async fn setup_nydusd(&self, h: &dyn Hypervisor) -> Result<()> {
        ...
        let sock_path = generate_sock_path(&jailer_root);
        let api_sock_path = self.generate_api_sock_path(&jailer_root);
        ...
        let source_path_str = source_path
            .to_str()
            .ok_or_else(|| anyhow!("failed to convert source path to string"))?
            .to_string();
...
impl NydusShareFs for ShareVirtioFsNydus {
    async fn mount_rafs(
        ...
        self.do_mount_rafs_with_overlay(&rafs_mnt, rafs_meta, config, overlay_config)
            .await
