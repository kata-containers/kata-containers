...
impl CpuResource {
    ...
    pub(crate) async fn update_cpu_resources(
        ...
        let curr_vcpus = self
            .do_update_cpu_resources(vcpu_required as f32, op, hypervisor)
            .await?;
...
mod tests {
    ...
    async fn test_divisible_periods() {
        ...
        assert_eq!(cpu_resource.calc_cpu_resources().await.unwrap(), 3.0);
        ...
        assert_eq!(cpu_resource.calc_cpu_resources().await.unwrap(), 4.0);
    }
    ...
    async fn test_indivisible_periods() {
        ...
        let actual = cpu_resource.calc_cpu_resources().await.unwrap();
        let expected = 4.0; // pure quota sum, no default_vcpu added
        assert!(
            (actual - expected).abs() < EPSILON,
            "case1: got {}, expect {}",
            actual,
        ...
        let expected = (1_000_000.0 + (900_000.0 * (1_000_000.0 / 299_999.0))) / 1_000_000.0;
    ...
    async fn test_fractional_default_vcpus() {
        ...
        assert_eq!(cpu_resource.calc_cpu_resources().await.unwrap(), 0.25);

        let mut cpu_resource = get_cpu_resource_with_default_vcpus(default_vcpus);
        add_linux_container_cpu_resources(&mut cpu_resource, vec![(500_000, 1_000_000)]).await;
        assert_eq!(cpu_resource.calc_cpu_resources().await.unwrap(), 0.5);
        ...
        let mut cpu_resource = get_cpu_resource_with_default_vcpus(default_vcpus);
        add_linux_container_cpu_resources(&mut cpu_resource, vec![(500_001, 1_000_000)]).await;
        assert_eq!(cpu_resource.calc_cpu_resources().await.unwrap(), 0.500001);
