...
/// KATA_LOCAL_DEV_TYPE creates a local directory inside the VM for sharing files between
/// containers.
const KATA_LOCAL_DEV_TYPE: &str = "local";
...
impl LocalStorage {
    pub(crate) fn new(m: &oci::Mount, sid: &str, cid: &str) -> Result<Self> {
        ...
        let local_storage = agent::Storage {
            driver: String::from(KATA_LOCAL_DEV_TYPE),
            driver_options: Vec::new(),
            source: String::from(KATA_LOCAL_DEV_TYPE),
            fs_type: String::from(KATA_LOCAL_DEV_TYPE),
            fs_group: None,
            options: dir_options,
        ...
        let mounts: Vec<oci::Mount> = if sid != cid {
            ...
            mount.set_typ(Some("local".to_string()));
...
pub(crate) fn is_local_volume(m: &oci::Mount) -> bool {
    get_mount_type(m).as_str() == KATA_LOCAL_DEV_TYPE
}
