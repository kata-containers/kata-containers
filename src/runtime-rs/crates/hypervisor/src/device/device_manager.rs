...
impl DeviceManager {
    ...
    async fn get_block_device(&self) -> BlockDeviceInfo {
...
pub async fn get_block_device(d: &RwLock<DeviceManager>) -> BlockDeviceInfo {
    d.read().await.get_block_device().await
}
...
mod tests {
    ...
    use crate::{
        device::{device_manager::get_block_device, DeviceConfig, DeviceType},
        qemu::Qemu,
