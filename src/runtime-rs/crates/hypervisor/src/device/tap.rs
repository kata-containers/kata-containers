...
pub struct Tap {
    ...
    pub tap_file: File,
    pub(crate) if_name: [i8; IFACE_NAME_MAX_LEN],
...
pub fn build_terminated_if_name(if_name: &str) -> Result<[i8; IFACE_NAME_MAX_LEN]> {
    ...
    // 0 is the null terminator for i8 type
    terminated_if_name[if_name_bytes.len()] = 0;
...
impl Tap {
    ...
    pub fn enable(&self) -> Result<()> {
        ...
        let ret = unsafe { ioctl_with_ref(&sock, c_ulong::from(SIOCSIFFLAGS), &mut ifr) };
        if ret < 0 {
            return Err(Error::IoctlError(IoError::last_os_error()));
        }
...
mod tests {
    ...
    fn test_tap_get_ifreq() {
        ...
        // Generate the expected name based on the current value of NEXT_IP
        let next_ip = NEXT_IP.load(Ordering::SeqCst) - 1; // Subtract 1 to get the previous allocated index
        let expected_name = format!("test_tap{next_ip}");
        // Compare with the expected name
        assert_eq!(expected_name, ifr_name);
    }
