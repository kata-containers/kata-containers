...
impl Platform {
    ...
    fn apply_q35_defaults(&mut self, topo: &HostTopology) {
        ...
        let has_gpus = topo
            .gpu_smmu_groups
            .iter()
            .any(|g| !g.pci_bus_addrs.is_empty());
        if !has_gpus {
            return;
        }
    ...
    fn apply_virt_defaults(&mut self, topo: &HostTopology) {
        ...
        self.objects.iommufd = Some(IommufdBackend {
            id: "iommufd0".to_owned(),
        });
        ...
        for (group_idx, group) in topo.gpu_smmu_groups.iter().enumerate() {
            ...
            for pci_addr in &group.pci_bus_addrs {
                ...
                for i in 0..8u32 {
                    self.objects
                        .acpi_links
                        .push(AcpiPciNodeLink::GenericInitiator {
    ...
    /// ordering because virt may reference `memory-backend=` on the machine line
    ...
    fn emit_q35_args(&self) -> Result<Vec<String>> {
        ...
        for (backend, node) in self
            ...
            .memory_backends
            .iter()
            .zip(self.objects.numa_nodes.iter())
        {
            args.push("-object".to_owned());
    ...
    ///   1. iommufd object
    ///   2. memory backends
    ///   3. -machine (references memory-backend= only without NUMA memdev=)
    ///   4. NUMA nodes
    ...
    fn emit_virt_args(&self) -> Result<Vec<String>> {
        ...
        let has_egm =
            self.machine.base().memory_backend.is_none() && self.objects.memory_backends.len() > 1;
        ...
        for root in &self.pci.roots {
            ...
            for port in &root.root_ports {
                ...
                if let Some(vfio) = &port.device {
                    args.push("-device".to_owned());
                    args.push(emit_vfio_grace(
                        vfio,
                        &port.id,
                        self.objects.iommufd.as_ref(),
                    ));
                }
            }
        }
        ...
        for link in self.objects.acpi_links.iter() {
            if let AcpiPciNodeLink::GenericInitiator { id, pci_dev, node } = link {
                args.push("-object".to_owned());
                args.push(format!(
                    "acpi-generic-initiator,id={id},pci-dev={pci_dev},node={node}"
                ));
            }
        }
        for link in self.objects.acpi_links.iter() {
            if let AcpiPciNodeLink::EgmMemory { id, pci_dev, node } = link {
                args.push("-object".to_owned());
                args.push(format!(
                    "acpi-egm-memory,id={id},pci-dev={pci_dev},node={node}"
                ));
            }
        }
...
fn format_memory(size: u64) -> String {
    if size.is_multiple_of(1 << 40) {
        format!("{}T", size >> 40)
    } else if size.is_multiple_of(1 << 30) {
        format!("{}G", size >> 30)
    } else {
        format!("{}M", size >> 20)
    }
...
fn emit_backend(backend: &MemoryBackend, id: &str) -> String {
    match backend {
        ...
        MemoryBackend::File {
            ...
            let mut s = format!(
                "memory-backend-file,id={id},size={},mem-path={path}",
                format_memory(*size)
            );
            if let Some(hn) = host_nodes {
                s.push_str(&format!(",host-nodes={hn}"));
...
fn emit_pxb(root: &PciRootComplex, default_bus: &str) -> String {
    let mut s = format!(
        "pxb-pcie,id={},bus={},bus_nr={}",
        root.id, default_bus, root.bus_nr
    );
...
fn emit_root_port(port: &PciRootPort, bus: &str) -> String {
    let mut s = format!(
        "pcie-root-port,id={},bus={},chassis={}",
        port.id, bus, port.chassis
    );
