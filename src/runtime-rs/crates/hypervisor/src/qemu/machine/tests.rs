...
fn apply_host_defaults_egm_adds_backends_and_links() {
    ...
    assert!(
        matches!(p.objects.memory_backends[1], MemoryBackend::File { ref path, .. } if path == "/dev/egm4")
    );
    assert!(
        matches!(p.objects.memory_backends[2], MemoryBackend::File { ref path, .. } if path == "/dev/egm5")
    );
