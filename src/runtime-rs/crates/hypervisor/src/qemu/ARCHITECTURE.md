...
**virt / Grace (aarch64)** — backends precede the machine line because a
virt machine without NUMA memory assignments can reference `memory-backend=<id>`.
When any NUMA node carries `memdev=`, omit that machine property: QEMU rejects
...
3. `-machine virt,...` (`memory-backend=m0` only without NUMA `memdev=`)
