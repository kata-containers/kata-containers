...
use crate::firecracker::inner_hypervisor::FC_API_SOCKET_NAME;
...
impl FcInner {
    ...
    pub(crate) async fn prepare_vmm(&mut self, netns: Option<String>) -> Result<()> {
        ...
        match self.jailed {
            ...
            false => {
                debug!(sl!(), "[Firecracker] Running non-Jailed");
                cmd = Command::new(&self.config.path);
                cmd.args(["--api-sock", &self.asock_path]);
            }
        }
        debug!(sl!(), "[Firecracker] Exec: {:?}", cmd);
        ...
        match child.id() {
            Some(id) => {
                ...
                info!(
                    sl!(),
                    "[Firecracker] VMM spawned successfully: PID: {:?}, current TID: {:?}",
                    id,
                    cur_tid
                );
                self.pid = Some(id);
    ...
    pub(crate) fn resize_memory(&mut self, new_mem_mb: u32) -> Result<(u32, MemoryConfig)> {
        warn!(
            sl!(),
            "[Firecracker] memory size unchanged, requested: {:?} Not implemented", new_mem_mb
        );
        Ok((
            0,
            MemoryConfig {
    ...
    pub(crate) fn set_guest_memory_block_size(&mut self, size: u32) {
        warn!(
            sl!(),
            "[Firecracker] guest memory block size unchanged, requested: {:?}, Not implemented",
            size
        );
    }
    ...
    pub(crate) fn guest_memory_block_size_mb(&self) -> u32 {
        warn!(
            sl!(),
            "[Firecracker] guest memory block size Not implemented"
        );
        0
    }
}
