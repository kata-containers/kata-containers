...
use super::FcInner;
use crate::firecracker::inner_hypervisor::{FC_AGENT_SOCKET_NAME, ROOT};
use crate::VmmState;
...
impl FcInner {
    pub(crate) async fn add_device(&mut self, device: DeviceType) -> Result<()> {
        if self.state == VmmState::NotReady {
            info!(
                sl!(),
                "[Firecracker] VMM not ready, queueing device {}", device
            );
        ...
        debug!(sl!(), "[Firecracker]: Add Device {} ", &device);
    ...
    pub(crate) async fn remove_device(&mut self, device: DeviceType) -> Result<()> {
        info!(sl!(), "[Firecracker]: Remove Device {} ", device);
        Ok(())
    }

    pub(crate) async fn update_device(&mut self, device: DeviceType) -> Result<()> {
        info!(sl!(), "[Firecracker] update device {:?}", &device);
        Ok(())
    }
