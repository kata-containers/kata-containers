...
use kata_types::{build_rootless_path, config::KATA_PATH};
use serde::de;
...
pub fn to_disk<T: serde::Serialize>(value: &T, sid: &str, jailer_path: &str) -> Result<()> {
    ...
    let mut path = match jailer_path {
        "" => scoped_join(build_rootless_path(KATA_PATH), sid)?,
...
{
    verify_id(sid).context("failed to verify sid")?;
    let mut path = scoped_join(build_rootless_path(KATA_PATH), sid)?;
    if path.exists() {
        path.push(PERSIST_FILE);
