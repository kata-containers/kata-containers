...
impl Process {
    ...
    async fn run_io_wait(
        ...
        tokio::spawn(async move {
            ...
            // Match Go runtime behavior: even if wait_process fails (e.g., VM died),
            // we still set status to Stopped. This ensures that subsequent Kill()
            // calls see the process as already stopped and return success.
            // See: src/runtime/pkg/containerd-shim-v2/wait.go lines 50-90
            let exit_code = match agent.wait_process(req).await {
                ...
                Err(e) => {
                    ...
                    // This matches Go behavior where status is set even on error
                    error!(logger, "failed to wait process {:?}", e);
                    255 // Use exit code 255 for unknown exit, matching Go's exitCode255
                }
            };
