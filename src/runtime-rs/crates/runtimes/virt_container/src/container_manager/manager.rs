    ...
    async fn kill_process(&self, req: &KillRequest) -> Result<()> {
        ...
        let c = match containers.get(container_id) {
            ...
            None => {
                ...
                if req.signal == libc::SIGKILL as u32 || req.signal == libc::SIGTERM as u32 {
                    warn!(
                        sl!(),
        ...
        c.kill_process(&req.process, req.signal, req.all)
            .await
            .or_else(|err| {
                let is_termination_signal =
                    req.signal == libc::SIGKILL as u32 || req.signal == libc::SIGTERM as u32;
                ...
                if is_termination_signal && is_expected_error {
                    warn!(
                        sl!(),
