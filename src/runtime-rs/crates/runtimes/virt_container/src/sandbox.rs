...
impl VirtSandbox {
    ...
    async fn prepare_pcie_port_devices(&self) -> Option<PortDeviceConfig> {
        ...
        let (port_type, total_ports) = dm.get_pcie_topology()?.get_pcie_port()?;

        // Return early if there are no ports
        if total_ports == 0 {
            info!(sl!(), "No PCIe ports available for VM.");
            return None;
        }

        // Print the preparation of PCIe devices
        info!(
            sl!(),
            "Preparing PCIe {:?} with {} devices for VM.", port_type, total_ports
        );

        // Create and return the port configuration
