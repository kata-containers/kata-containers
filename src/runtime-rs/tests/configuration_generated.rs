...
fn generate_configs(runtime_dir: &Path, output_dir: &Path, arch: &str) {
    ...
    let output = Command::new("make")
        ...
        // aarch64-options.mk does not set CLHCMD yet, so CONFIG_CLH is omitted
        // from CONFIGS unless the binary name is provided on the command line.
...
#[test]
fn generated_direct_kernel_configs_do_not_inherit_qemu_firmware() {
    ...
    for arch in ["x86_64", "aarch64"] {
        ...
        assert_firmware(&output_dir.join("configuration-clh-runtime-rs.toml"), "");
        assert_firmware(
            &output_dir.join("configuration-clh-azure-runtime-rs.toml"),
            "",
        );
    }

    assert_firmware(
        &temp_dir.0.join("x86_64/configuration-qemu-runtime-rs.toml"),
        "",
    );
    assert_firmware(
        &temp_dir
            .0
            .join("aarch64/configuration-qemu-runtime-rs.toml"),
