...
func (c *Container) pullImage(ctx context.Context, image string, new_container_id string) error {
	if err := c.checkSandboxRunning("pullImage"); err != nil {
		return err
	}
	...
	if err := c.sandbox.agent.pullImage(ctx, c.sandbox, *c, image, new_container_id); err != nil {
		return err
	}
