...
func (s *Sandbox) PullImage(ctx context.Context, ID string, image string, containerID string) error {
	// Fetch the container.
	c, err := s.findContainer(ID)
	if err != nil {
		return err
	}

	// Pull the image.
	if err := c.pullImage(ctx, image, containerID); err != nil {
		return err
	}
