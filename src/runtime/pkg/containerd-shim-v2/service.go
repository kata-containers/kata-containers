...
func shouldUnshareMountNs(args []string, id string) bool {
	// HACK: this heuristicly guesses that args were generated by
	// newCommand().
	last_arg := args[len(args)-1]
	return last_arg == id || last_arg == "-debug"
}
...
func New(ctx context.Context, id string, publisher cdshim.Publisher, shutdown func()) (cdshim.Shim, error) {
	...
	if shouldUnshareMountNs(os.Args, id) {
		if err := setupMntNs(); err != nil {
			return nil, fmt.Errorf("failed to setup mount namespace : %v", err)
		}
	}
...
func newCommand(ctx context.Context, id, containerdBinary, containerdAddress string) (*sysexec.Cmd, error) {
	...
	// The only purpose of this panic is to catch at development time
	// any change that would break the heuristic in shouldUnshareMountNs().
	if !shouldUnshareMountNs(args, id) {
		panic("shouldUnshareMountNs() is out-of-sync with NewCommand()")
	}
