...
func getDeviceSpec(ctx context.Context, socket string, ann map[string]string) ([]string, error) {
	...
	if podRes == nil {
		return nil, wrappedError(fmt.Errorf("PodResources is nil"))
	}

	// Process results
	shimLog.Infof("@@@ Pod: %s/%s\n", podRes.Namespace, podRes.Name)
