...
func (vfioDev VFIODevice) QemuParams(config *Config) []string {
	...
	// [coldplug-vf-fix] Emit `id=` for the QEMU vfio-pci device so that
	// callers (notably the cold-plug path) can later resolve the
	// device's guest PCI path via QMP `qom-get` keyed on this ID.
