...
fn should_process_source_path(source_path: &str) -> bool {
    // Base path to check
...
async fn cdh_handler(oci: &mut Spec) -> Result<()> {
    ...
    for m in mounts.iter_mut() {
        if let Some(source_path) = m.source().as_ref().and_then(|p| p.to_str()) {
            ...
            if should_process_source_path(source_path) {
                info!(
                    sl(),
...
"#;
    ...
    #[tokio::test]
    async fn test_should_process_source_path() {
        #[derive(Debug)]
        struct TestData<'a> {
            source_path: &'a str,
            result: bool,
        ...
        for (i, d) in tests.iter().enumerate() {
            let msg = format!("test[{}]: {:?}", i, d);
            let result = should_process_source_path(d.source_path);
            assert_eq!(d.result, result, "{}", msg);
        }
    }
}
