...
impl AgentService {
    ...
    async fn do_create_container(
        ...
        let process = match confidential_data_hub::is_cdh_client_initialized() {
            true => {
                let new_p = crate::confidential_data_hub::image::get_process(
                    p,
                    &oci,
                    req.storages.clone(),
                )?;
            ...
            false => Process::new(&sl(), p, cid.as_str(), true, pipe_size, proc_io)?,
        };

        // if starting container failed, we will do some rollback work
        // to ensure no resources are leaked.
        if let Err(err) = ctr.start(process).await {
