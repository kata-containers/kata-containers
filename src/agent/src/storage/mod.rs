...
async fn update_storage_device(
    ...
    if let Err(device) = sandbox.lock().await.update_sandbox_storage(mount_point, device) {
        error!(logger, "failed to update device for storage"; "mount-point" => mount_point);
        if let Err(e) = sandbox.lock().await.remove_sandbox_storage(mount_point).await {
        ...
        return Err(anyhow!("failed to update device for storage: {}", mount_point));
    }
    Ok(())
}
