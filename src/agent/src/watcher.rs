...
impl BindWatcher {
    ...
    fn valid_watchable_mount_path(path: &Path) -> bool {
        ...
        let mut uid_bytes = uid.bytes();
        if !matches!(uid_bytes.next(), Some(b'1'..=b'9'))
            || !uid_bytes.all(|byte| byte.is_ascii_digit())
        {
            return false;
        }
...
mod tests {
    ...
    fn isolated_watchable_mount_path(dir: &Path, passthrough: bool) -> PathBuf {
        let path = if passthrough {
            WATCH_MOUNT_POINT_PATH_PASSTHROUGH
        } else {
            WATCH_MOUNT_POINT_PATH
    ...
    #[test]
    fn derive_watchable_mount_path() {
        for rootless_uid in [None, Some(1002)] {
    ...
    #[test]
    fn reject_invalid_watchable_mount_paths() {
        for mount_point in [
            PathBuf::from("relative/watchable/config"),
            PathBuf::from("/run/kata-containers/shared/containers/watchable/../config"),
            PathBuf::from("/tmp/watchable/config"),
            PathBuf::from("/tmp/run/kata-containers/shared/containers/watchable/config"),
            PathBuf::from(
                "/run/user/not-a-uid/run/kata-containers/shared/containers/watchable/config",
            ),
            PathBuf::from("/run/user/0/run/kata-containers/shared/containers/watchable/config"),
            PathBuf::from("/run/user/01002/run/kata-containers/shared/containers/watchable/config"),
            PathBuf::from(
                "/run/user/1002/tmp/run/kata-containers/shared/containers/watchable/config",
            ),
        ] {
            let mounts = [test_watchable_storage(&mount_point)];
    ...
    async fn create_tmpfs() {
        ...
        for passthrough in [false, true] {
            let mount_point = isolated_watchable_mount_path(dir.path(), passthrough);
