...
pub fn pcipath_from_qom_path(qom_path: &str) -> Result<(&str, pci::Path)> {
    ...
    //
    // let mut pcipath_parts = pcipath.splitn(2, '/');
    // let root_complex = pcipath_parts.next().unwrap();
    // let pcipath = pcipath_parts.next().unwrap();
    //
    let pci_path = pci::Path::from_str(qom_path)
        .with_context(|| format!("Failed to parse PCI path from QOM path '{}'", qom_path))?;
    Ok(("00", pci_path))
}
