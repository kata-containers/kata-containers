...
impl SystemdInterface for DBusClient {
    fn start_unit(
        ...
        let proxy = self
            .build_proxy()
            .with_context(|| format!("could not build proxy"))?;
        ...
        proxy
            .start_transient_unit(unit_name, UNIT_MODE, &properties, &vec![])
    ...
    fn get_version(&self) -> Result<String> {
        ...
        let systemd_version = proxy
            .version()
            .with_context(|| format!("failed to get systemd version"))?;
        Ok(systemd_version)
    }
    ...
    fn add_process(&self, pid: i32, unit_name: &str) -> Result<()> {
        ...
        proxy
            .attach_processes_to_unit(unit_name, "/", &vec![pid as u32])
            .with_context(|| format!("failed to stop unit {}", unit_name))?;
