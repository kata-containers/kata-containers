...
impl SystemdInterface for DBusClient {
    fn start_unit(&self, pid: i32, parent: &str, cg_hierarchy: &CgroupHierarchy) -> Result<()> {
        ...
        proxy
            ...
            .with_context(|| format!("failed to start transient unit {}", self.unit_name))?;
    ...
    fn set_properties(&self, properties: &Properties) -> Result<()> {
        ...
        proxy
            ...
            .with_context(|| format!("failed to set unit properties {}", self.unit_name))?;
    ...
    fn add_process(&self, pid: i32) -> Result<()> {
        ...
        proxy
            ...
            .with_context(|| format!("failed to add process {}", self.unit_name))?;
    ...
    fn unit_exists(&self) -> Result<bool> {
        let proxy = self
            .build_proxy()
            .with_context(|| format!("Checking if systemd unit {} exists", self.unit_name))?;

        Ok(proxy.get_unit(&self.unit_name).is_ok())
    }
