...
pub mod tests {
    ...
    use cgroups::FsManager;

    use crate::cgroups::container::{ContainerCgroupManager, ContainerCgroupManagerInner};
    use crate::cgroups::sandbox::SandboxCgroupManager;

    impl ContainerCgroupManager {
        pub fn new_for_test(path: &str, create: bool) -> Self {
            let mut manager = FsManager::new(path).unwrap();
            if create {
                manager.create_cgroups().unwrap();
            }

            let inner = ContainerCgroupManagerInner {
                sandbox_cgroup_manager: Arc::new(SandboxCgroupManager::default()),
                cgroup_manager: Box::new(manager),
            };

            ContainerCgroupManager {
