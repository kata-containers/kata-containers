...
use protocols::trans::{from_namespace_type, from_posix_rlimit_type};
...
fn do_init_child(cwfd: RawFd) -> Result<()> {
    ...
    for ns in &nses {
        let s = NAMESPACES.get(&from_namespace_type(&ns.typ()));
        if s.is_none() {
    ...
    for rl in process_rlimits.iter() {
        ...
        setrlimit(
            Resource::from_str(from_posix_rlimit_type(&rl.typ()))?,
    ...
    if to_new.contains(CloneFlags::CLONE_NEWUTS) {
        unistd::sethostname(spec.hostname().as_ref().unwrap())?;
    }
    ...
    if init {
        ...
        if let Some(hooks) = spec.hooks().as_ref() {
            ...
            create_container_states.execute_hooks(
                &hooks.create_container().clone().unwrap_or_default(),
                Some(state.clone()),
            )?;
        }
    }
    ...
    if init {
        ...
        if let Some(hooks) = spec.hooks().as_ref() {
            ...
            start_container_states.execute_hooks(
                &hooks.start_container().clone().unwrap_or_default(),
                Some(state),
            )?;
        }
    }
...
impl BaseContainer for LinuxContainer {
    ...
    async fn destroy(&mut self) -> Result<()> {
        ...
        if let Some(hooks) = spec.hooks().as_ref() {
            ...
            hook_states.execute_hooks(&hooks.poststop().clone().unwrap_or_default(), Some(st))?;
        }
    ...
    async fn exec(&mut self) -> Result<()> {
        ...
        if let Some(hooks) = spec.hooks().as_ref() {
            ...
            let mut hook_states = HookStates::new();
            hook_states.execute_hooks(&hooks.poststart().clone().unwrap_or_default(), Some(st))?;
        }
...
pub fn update_namespaces(logger: &Logger, spec: &mut Spec, init_pid: RawFd) -> Result<()> {
    ...
    if let Some(namespaces) = linux.namespaces_mut().as_mut() {
        for namespace in namespaces.iter_mut() {
            if TYPETONAME.contains_key(from_namespace_type(&namespace.typ())) {
                let ns_path = format!(
                    "/proc/{}/ns/{}",
                    init_pid,
                    TYPETONAME
                        .get(from_namespace_type(&namespace.typ()))
                        .unwrap()
                );
...
fn get_pid_namespace(logger: &Logger, linux: &Linux) -> Result<PidNs> {
    ...
    for ns in &linux_namespaces {
        if from_namespace_type(&ns.typ()) == "pid" {
...
fn is_userns_enabled(linux: &Linux) -> bool {
    linux
        ...
        .unwrap_or_default()
        .iter()
        .any(|ns| from_namespace_type(&ns.typ()) == "user" && ns.path().is_none())
}
...
impl LinuxContainer {
    pub fn new<T: Into<String> + Display + Clone>(
        ...
        let linux = spec.linux().as_ref().unwrap();
        let linux_cgroups_path = match linux.cgroups_path() {
            Some(cgrpath) => cgrpath.display().to_string(),
            None => String::new(),
        };
