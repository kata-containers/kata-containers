...
use protocols::trans::from_namespace_type;
use regex::Regex;
...
fn contain_namespace(nses: &[LinuxNamespace], key: &str) -> bool {
    for ns in nses {
        if from_namespace_type(&ns.typ()) == key {
            return true;
        }
    }
...
fn security(oci: &Spec) -> Result<()> {
    ...
    if !contain_namespace(linux.namespaces().as_ref().unwrap_or(&default_vec), "mount") {
...
fn sysctl(oci: &Spec) -> Result<()> {
    ...
    for (key, _) in sysctl_hash.iter() {
        ...
        if SYSCTLS.contains_key(key.as_str()) || key.starts_with("fs.mqueue.") {
            if contain_namespace(linux.namespaces().as_ref().unwrap_or(&default_vec), "ipc") {
                continue;
            } else {
