	...
	if volumeCtx[utils.KataContainersDirectVolumeType] == utils.SpdkVolumeTypeName {
		...
		if _, err := os.Stat(backingFile); os.IsNotExist(err) {
			...
			if err := f.Truncate(sizeBytes); err != nil {
				return nil, status.Errorf(codes.Internal, "failed to truncate backing file: %v", err)
			}

			klog.Infof("[SPDK] created new sparse backing file %s (%d bytes)", backingFile, sizeBytes)
		} else {
			klog.Infof("[SPDK] reuse existing backing file %s", backingFile)
		}

		// bdevName := fmt.Sprintf("bdev-%s", volumeID[:8])
		...
		if _, err := spdkrpc.CallSPDK("bdev_aio_create", params); err != nil {
...
func (dv *directVolume) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	if vol.Metadata["type"] == utils.SpdkVolumeTypeName {
		...
		if bdevName != "" {
			_, err := spdkrpc.CallSPDK("bdev_aio_delete", map[string]any{
				"name": bdevName,
			})
			if err != nil {
				...
				klog.Warningf("SPDK bdev %s already absent: %v", bdevName, err)
			} else {
