...
impl Container {
    ...
    pub fn get_process_fields(&self, process: &mut policy::KataProcess, is_pause_container: bool) {
        debug!(
        ...
        if let Some(context) = &self.securityContext {
            ...
            if let Some(uid) = context.runAsUser {
                ...
                policy::set_process_user_gid(process, is_pause_container, gid);
            }

            if let Some(gid) = context.runAsGroup {
                debug!("get_process_fields: runAsGroup = {:?}", gid);
                policy::set_process_user_gid(process, is_pause_container, gid.try_into().unwrap());
            }
