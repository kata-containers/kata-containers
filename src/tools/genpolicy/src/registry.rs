...
impl Container {
    pub async fn new(config: &Config, image: &str) -> Result<Self> {
        ...
        match client.pull_manifest_and_config(&reference, &auth).await {
            Ok((manifest, digest_hash, config_layer_str)) => {
                ...
                // Find the last layer with an /etc/* file,
    ...
    fn get_gid_from_group_name(&self, name: &str) -> Result<u32> {
        ...
        match parse_group_file(self.group.clone()) {
            Ok(records) => {
                if let Some(record) = records.iter().find(|&r| r.name == name) {
                    return Ok(record.gid);
                } else {
                    return Err(anyhow!("Failed to find name {} in /etc/group", name));
                }
            }
            Err(inner_e) => {
                return Err(anyhow!("Failed to parse /etc/group - error {inner_e}"));
            }
        }
    }
    ...
    fn parse_group_string(&self, group: &str) -> u32 {
        ...
        match group.parse::<u32>() {
            ...
            Err(outer_e) => {
                ...
                return self.get_gid_from_group_name(group).unwrap_or(0);
            }
        }
    }
...
fn parse_group_file(group: String) -> Result<Vec<GroupRecord>> {
    let mut records = Vec::new();
