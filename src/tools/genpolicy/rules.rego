...
allow_container_image_storage(p_storages, layer_ids, root_hashes) {
    print("allow_container_image_storage 1: start")

    policy_data.common.enable_image_layer_storage == true

    some overlay_storage in p_storages
    overlay_storage.driver == "overlayfs"

    print("allow_container_image_storage 1: overlay_storage =", overlay_storage)
    count(overlay_storage.options) == 2

    layer_ids := split(overlay_storage.options[0], ":")
    print("allow_container_image_storage 1: layer_ids =", layer_ids)

    root_hashes := split(overlay_storage.options[1], ":")
    print("allow_container_image_storage 1: root_hashes =", root_hashes)
}
allow_container_image_storage(p_storages, layer_ids, root_hashes) {
    print("allow_container_image_storage 2: start")
    policy_data.common.enable_image_layer_storage == false
    print("allow_container_image_storage 2: true")
}
