...
!!! tip "Prefer your distribution's package, if there is one"
    If your distribution packages Kata Containers, install that version so
    it is updated through the normal package channel. The instructions below
    install the newest upstream release, which is **your** responsibility to
    keep up to date.
...
version that does *not* contain a dash (`-`) — a dash indicates a pre-release.
