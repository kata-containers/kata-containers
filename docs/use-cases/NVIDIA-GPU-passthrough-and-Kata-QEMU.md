...
- **nvidia-sandbox-device-plugin** (see the
  ...
  - Discovering NVIDIA GPUs, their capabilities, and advertising these to
    ...
    `nvidia.com/pgpu` resources will appear for the node and GPU DeviceIDs
    will be registered with kubelet). These GPUs can thus be allocated as
    container resources in your pod manifests.
...
When this happens, kubelet calls into the sandbox device plugin to allocate
a GPU. The sandbox device plugin returns a GPU device ID and outer runtime
CDI annotations to the kubelet. The kubelet uses the GPU device ID for
tracking of allocated GPUs and includes the CDI annotations in the OCI spec
when scheduling the pod through containerd. Containerd in turn invokes the
Kata runtime with the OCI spec.

### Kata runtime

The Kata runtime receives the OCI spec including the CDI annotations from
containerd. The orchestration flow then differs between cold-plug (TEE) and
hot-plug (non-TEE) scenarios:

For NVIDIA GPU TEE workloads, the GPU is attached at VM launch time.
However, the kubelet passes the device information via the OCI spec only at
container creation time. Therefore, the Kata runtime queries kubelet's Pod
Resources API using the annotations to acquire the GPU device ID allocated
by the sandbox device plugin. The runtime thus realizes that, for example,
the GPU with ID `GPU0` is requested. The runtime then consults the host CDI
specifications and determines the device path the GPU is backed by (e.g.,
`/dev/vfio/devices/vfio0`). Then, the runtime resolves the device's PCI BDF
(e.g., `0000:21:00`) and cold-plugs the GPU by launching QEMU with relevant
parameters for device passthrough (e.g.,
...
For non-TEE NVIDIA GPU workloads (hot-plug), the runtime attaches the GPU to
the pod VM with QEMU before container creation, but after the VM was created
(using a QMP `device_add` command). At this point in time, kubelet has
already passed the device information via the OCI spec. Therefore, querying
the Pod Resources API is not necessary - the device info is already available
in the OCI spec. For the container creation request, the runtime then also
provides kata-agent with relevant device information - most importantly, the
device PCI BDF - indicating which devices are expected to be hot-plugged.
This enables the kata-agent to wait for relevant hot-plug events after being
started, in more detail, to wait for a `uevent` using a `PciMatcher` looking
for new entries under the `/sys/devices/` path.

As part of the create container request, the runtime also creates "inner
runtime" CDI annotations. This provides kata-agent with the relevant
information on how to handle the passthrough devices and on to what container
to attach the device. The inner runtime annotations are pairs consisting of
`cdi.k8s.io/vfio<num>` keys (an individual key for each GPU derived from the
host VFIO device path, e.g., `/dev/vfio/devices/vfio1`) and
`nvidia.com/gpu=<index>` values (referencing the device in the guest CDI
spec).
...
In the second stage, a chiselled build is performed: Only relevant contents
...
filesystem into the NVIDIA initrd and image collaterals.
...
process, scans for NVIDIA GPUs on the PCI bus, loads the NVIDIA kernel
modules, waits for driver initialization, creates the device nodes, and
initializes the GPU hardware (using the `nvidia-smi` binary). NVRC also
...
into the container which receives the passthrough GPU. Once the container is
started, this provides the complete GPU software stack so that a CUDA
container can run its workloads. In the next step, NVRC forks the Kata agent
which waits for the Kata runtime's create container request. See the
...
parses the "inner runtime" CDI annotation. For example, given the annotation
`"cdi.k8s.io/vfio1": "nvidia.com/gpu=0"`, the agent looks up device `0` in
the guest CDI specification with `kind: nvidia.com/gpu`.
...
page, provide guidance on selecting proper hardware and on properly
configuring its firmware and OS.
...
version and Kata's current supported Kubernetes version (in which the Pod
Resources API is enabled by default - for older Kubernetes versions, this
API may require explicit enablement). We set this cluster up using the
`deploy_k8s` function from `tests/integration/kubernetes/gha-run.sh` as
follows:
...
> to deallocate your pod before it transitions from the 'container created'
...
> your nodes to be labelled for GPU VM passthrough. Remove this parameter if
...
Use the previously described node labelling approach to transition between
