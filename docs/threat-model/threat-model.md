...
the default settings provided when integrating Kata with the QEMU, Firecracker
and Cloud Hypervisor VMMs in the following sections.
...
In Firecracker and Cloud Hypervisor, vsock is backed by a unix-domain-socket in
the hosts userspace.
...
manipulate power management features, causing denial of service (DoS) through
