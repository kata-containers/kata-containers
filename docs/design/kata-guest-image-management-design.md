...
flowchart LR
    ...
    E-->|2\. Pull image metadata| Containerd
    ...
    E[Container Images Registry]-->|6\. Pull image with image info|CDH1
...
sequenceDiagram
    par Hosts Side
        ...
        runtime.fs_share_linux->>runtime.kata_agent: ShandleVirtualVolumeStorageObject(c,...,KataVolumeType)
    ...
    and Guest Side
        ...
        Confidential Data Hub->>agent.confidential_data_hub: ret: manifest_digest
...
`ImagePullHandler` delegates the image pulling operation to the `confidential_data_hub.pull_image()` that is going to create the image's bundle directory on the guest filesystem and, in turn, class Confidential Data Hub to in fact fetch and uncompress the image's bundle. 
