...
Kata Containers is an open source community working to build a secure container [runtime](#runtime) with lightweight virtual machines (VM's) that feel and perform like standard Linux containers, but provide stronger [workload](#workload) isolation using hardware [virtualization](#virtualization) technology as a second layer of defence.
...
We can now spawn a Kata container by submitting a request to containerd like so:

```
$ ctr image pull quay.io/libpod/ubuntu:latest
$ ctr run --runtime "io.containerd.kata.v2" --rm -t "quay.io/libpod/ubuntu:latest" foo sh
# echo hello
hello
```

Because containerd is aware of the Kata runtimes, we can also spawn Kubernetes pods:
