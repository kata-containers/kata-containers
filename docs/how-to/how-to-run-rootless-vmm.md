...
Turn `hypervisor.rootless` on for each shim that should run unprivileged. It
requires `deploymentMode: job`, which is the default — the DaemonSet has no
privileged host-root stage to provision devices from, and the chart refuses the
combination rather than configuring sandboxes that could not start.
...
1. Turns `rootless = true` on for that shim, through a `25-rootless.toml`
   drop-in.
2. Reconciles the devices the shims that asked for it need, and records the
   result as a udev rule, so it survives a reboot or the device being
   re-created:
...
The rule only lists the devices the rootless shims actually need: `/dev/sev`
appears when an SNP shim is rootless, `/dev/uv` when `qemu-se-runtime-rs` is. An
SNP shim left privileged contributes nothing, so a node keeps `/dev/sev` as it
found it. Groups are created with the node's own `groupadd --system`, so they
land in the node's system-GID range.

A device the node does not have is skipped, so a plain node with no `/dev/sev`
is an ordinary case and not an error. A device that *already* grants unprivileged access is left exactly as
