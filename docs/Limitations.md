...
filesystem metadata or place kubelet's volume data on a smaller dedicated
filesystem. Increasing `sizeLimit` avoids premature eviction but does not
reduce the backing image's metadata allocation. There is currently no Kata
configuration option that caps the block-backed `emptyDir` image size.
