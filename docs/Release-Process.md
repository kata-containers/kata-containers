...
The Kata Containers project uses [semantic versioning](http://semver.org/) for
all releases. Semantic versions are comprised of three fields in the form:
...
both** and which **changes the behavior from the previous release**
(incompatible with previous releases).

A major release will also likely require a change of the container manager
version used, -for example Containerd or CRI-O. Please refer to the release
...
details). Bug fixes are released as part of `MINOR` or `MAJOR` releases only.
`PATCH` is always `0`.
...
recently trailed setting the `main` branch to read only whilst the release
action runs.
