...
@test "Expose IP Address" {
	...
	# Create deployment
	pcl	"${pod_config_dir}/deployment-expose-ip.pcl" | kubectl create -f -
	...
	# Expose deployment
	kubectl expose deployment/${deployment} --name=${service}
	...
	cmd="kubectl exec -it ${pod_name} -- curl http://$svcip:$svcport/echo?msg=${echo_msg} | grep \"$echo_msg\""
